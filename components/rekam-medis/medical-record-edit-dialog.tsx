...
import { updateMedicalRecord, MedicalRecordItem } from '@/app/rekam-medis/actions'
import { formatNoRM } from '@/lib/patient-utils'

interface MedicalRecordEditDialogProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: (updated: MedicalRecordItem) => void
...
export function MedicalRecordEditDialog({
  ...
  useEffect(() => {
    if (record) {
      ...
      setSecondaryDiagnosis(record.secondaryDiagnosis || '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.record) {
        onSuccess(res.record as MedicalRecordItem)
      } else {
        if (res.errors) {
          setErrors(res.errors)
        } else {
          setGeneralError(res.error || 'Gagal memperbarui rekam medis')
        }
      }
    } catch (err: any) {
      setGeneralError(err.message || 'Terjadi kesalahan sistem saat memperbarui rekam medis')
    } finally {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/20">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Pasien: <span className="font-semibold text-foreground">{record.patient.name}</span> ({noRM})
            </p>
          </div>
          <button
            type="button"
